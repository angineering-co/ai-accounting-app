...
- `app/pay/[token]/page.tsx`：公開路由，用 `checkout_token` 查回該筆、產生並寫回 `merchant_trade_no`、server render 自動送出表單。
- `proxy.ts` 放行 `/pay`。
...
- 一個「產生收款連結」的小工具（先可純內部 / 手動）：選 client（或留空＝訂金）、`type`、金額、品項 → 建 pending row + `checkout_token` → 產出可寄出的 `/pay/<token>` 連結。
