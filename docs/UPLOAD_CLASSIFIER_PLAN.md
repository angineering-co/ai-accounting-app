...
- 明確的 per-document 分類生命週期：`accepted`（已跑且採納）/ `pending`（不一致或中低信心，待處理）/
  `unclassified`（失敗 / outage / 檔案過大，待重跑）/ `resolved`（已處理）；worker 一律寫入，不靠「沒值」推斷
- 信心分級：high-confidence 且一致 → `accepted`；中 / 低信心或不一致 → `pending`（事務所複核）
- `unclassified` 文件可**批次重跑**（Gemini 恢復後一次補跑），失敗不靜默丟棄
...
classification-worker (獨立 queue / worker)：
   ...
   → UPDATE documents.classifier_hint (一律寫 review_status)：
        高信心且一致                          → 'accepted'
        invoice/allowance 不一致 或 中/低信心   → 'pending'
        其他文件被判像 VAT (反向檢查)            → 'pending'（/documents chip，§5.6）
        失敗 / outage / 檔案過大 (重試耗盡)      → 'unclassified'（可批次重跑）
   worker 全程不碰子表
        │
        ▼
事務所員工處理（客戶看不到任何分類結果）：
...
**故障處理（不靜默丟棄）**：分類失敗時文件**照常走完上傳流程**（不阻擋），但**不**當成「已驗證」。
worker 先靠 pgmq retry / dead-letter 吸收暫時性失敗；重試耗盡（或檔案過大）才把 `review_status`
標為 `unclassified` 並寫 `error`。`unclassified` 是**可見、可批次重跑**的狀態（Gemini 恢復後一次補跑），
不像「靜默通過」會讓「其實是發票卻沒驗證」的文件無聲消失。為避免 outage 時暴量湧入，`unclassified`
與「分類不一致」**分開呈現**（§6.4），且以重跑為主、非要求逐筆人工處理。至於「從未跑過」
（`classifier_hint IS NULL`）：靠 enqueue 可靠性 + dead-letter；必要時以週期性 sweep 找「active 但
長時間仍 NULL」的文件補跑（NULL 無法入 §4.1 partial index）。
...
COMMENT ON COLUMN documents.classifier_hint IS '分類器判決與處理狀態 (verdict + review_status)';

-- 檢查點 / 重跑 / 計數用 partial index；同時涵蓋 pending（待處理）與 unclassified（待重跑），
-- 兩者都是「需被看見」的狀態。只索引這兩種 active 文件，極小。
CREATE INDEX idx_documents_classifier_actionable
  ON documents (client_id)
  WHERE classifier_hint->>'review_status' IN ('pending', 'unclassified') AND status = 'active';
```
...
```ts
{
  ...
  // 處理狀態 (worker 一律寫; classifier_hint IS NULL = 尚未跑)
  review_status: 'accepted' | 'pending' | 'unclassified' | 'resolved',
...
> **狀態語意**：`accepted`＝已跑且採納（無需動作，但留痕證明跑過）；`pending`＝待事務所處理
> （invoice/allowance 不一致或中低信心 → §6.4 檢查點；其他文件被反向檢查判像 VAT → §6.3 chip）；
> `unclassified`＝失敗 / outage / 過大，待批次重跑；`resolved`＝已處理。`classifier_hint IS NULL`
> 才代表「尚未跑」。`classifier_hint.doc_type` 為 audit 紀錄；`documents.doc_type` 才是 authoritative。
...
worker 跑在 Deno edge function，**不能用 `sharp`**（Node 原生模組，Deno 載不起來）。v1 不在 worker
內做影像處理：> 15MB 的檔案直接標 `review_status='unclassified'`（`error:'file_too_large'`），與其他
失敗一樣可批次重跑 / 人工核對。實務上 > 15MB 的發票 / 折讓極罕見；worker 內降採樣（改用 WASM 影像庫，
或上傳前先壓縮）留待有需求再做。
...
**批次重跑**：同一個 `enqueueClassification` 也用於補跑 `review_status='unclassified'` 的文件
（Gemini 恢復後）。走 §4.1 index 取 unclassified 集合（可按 client / 期別範圍）重新 enqueue；
重跑成功會把狀態改寫為 `accepted` / `pending`。
...
SET classifier_hint`，**一律寫入 `review_status`**（`accepted` / `pending`；重試耗盡或檔案過大則
`unclassified` + `error`）。**worker 永遠不碰子表。**
...
| `switchInOrOut` | 子表 `in_or_out` 設為指定 `target`（明確值，非 toggle）；OCR 跑過則重排 |
...
export async function switchInOrOut(documentId: string, target: 'in' | 'out'): Promise<void>;
...
- **OCR 重跑（必須顯式重排，不能只改狀態）**：擷取 prompt 被 `in_or_out` 帶偏，故 `switchInOrOut` /
  `convertDocType` / `promoteFromOther` 若在 OCR 之後才執行，原 `extracted_data` 已錯。OCR **不是**由
  `ocr_status` 觸發，而是顯式 enqueue（`bulk-extraction`）；因此這些動作除了把 `ocr_status` 設回
  `pending`，**還要顯式對該 document 重新 enqueue 擷取 job**。`convertDocType` 一律以「建全新目標子表
  + 重排 OCR」處理（不搬移 `extracted_data`）。
- **switchInOrOut 用明確 `target`**（非無參數 toggle），避免重複點擊 / 競態把狀態切回原樣。
...
```
經子表 `tax_filing_period_id = periodId` 關聯 `documents`，篩 `review_status IN ('pending','unclassified')`
且 `status='active'`（走 §4.1 index），回傳時分成「待確認」與「未能分類」兩組供 §6.4 顯示。childless
`other` 無期別，天然不在此查詢（其狀態在 §6.3 `/documents` 呈現）。
...
統一發票 / 折讓單」的高風險錯誤（誤標的真發票會跳過 OCR、永不進稅務資料）。判像 VAT 時 worker
標 `review_status='pending'`，由事務所 `/documents` 的反向檢查 chip 呈現可 promote（§6.3）。客戶端
不顯示此訊號。（childless `other` 無期別，故不進 §6.4 per-period 檢查點，只在 `/documents` 出現。）
...
> **實作修正（見 `UPLOAD_CLASSIFIER_PHASED_PLAN.md` PR-1a）**：本節原構想是把「其他文件」加進
> 3 個既有上傳入口。但 `components/document-upload-section.tsx`（portal 期別頁的 4 張固定
> type+inOrOut 卡、無類型選擇器）與 `components/invoice/invoice-upload-dialog.tsx`（firm 期別頁、
> 期別鎖定）皆為**期別範圍**的上傳面，而「其他文件」**無期別**，塞進去會洩漏期別語意。故改為：
> v1 先把「其他文件」上傳放在 **`/documents` 頁自身**（client 範圍、無期別、與管理同頁）；
> 桌機期別頁維持只放 VAT 文件卡，mobile FAB 的「其他文件」選項**刻意延後到最後一步**、且在分類器
> 反向檢查上線後才開（客戶曝光分階段，見 `UPLOAD_CLASSIFIER_PHASED_PLAN.md` 後續步驟 A / B）。
> 維持「期別頁＝屬於申報期的 VAT 文件、`/documents`＝其餘無期別文件」的分界。

選發票 / 折讓 → 現行 `createInvoice` / `createAllowance`（期別頁入口不變）；選其他文件 →
`createDocument`（於 `/documents` 頁）。上傳處理常式在 row 建立後一次性呼叫 `enqueueClassification`。
...
- **反向檢查 chip**（列上的一個小提示按鈕）：當某筆 `other` 文件被分類器判定其實像發票 / 折讓
  （`review_status='pending'` 且 verdict 為 invoice/allowance），該列顯示「看起來像發票 / 折讓，要轉嗎？」
  的 chip；點擊即開 `promoteFromOther`（選 in/out + 期別）。這是把「誤標為其他文件的真發票」救回來的主要入口。
...
- 另顯「未能分類：M」＝該期別 `review_status='unclassified'` 數，附「重跑」按鈕（§5.2 批次重跑）；與
  「待確認」**分開呈現**，避免 outage 時把暫時失敗誤當成大量不一致。
...
| `lib/services/document-classifier.ts` | NEW — `classifyDocument` + `enqueueClassification`（兼批次重跑 unclassified）|
| `lib/services/document-classifier.test.ts` | NEW — 單元測試 |
| `supabase/functions/classification-worker/` | NEW — 獨立 pgmq worker |
| `lib/services/document.ts` | 加 `resolveClassificationKeep` / `switchInOrOut(target)` / `convertDocType` / `demoteToOther` / `promoteFromOther`（皆事務所端，Drizzle 交易；switch/convert/promote 顯式重排 OCR）+ `getPeriodPendingClassifications`（含 unclassified）|
| `app/firm/[firmId]/client/[clientId]/documents/page.tsx` | 「其他文件」上傳控制（§6.1 修正：放在 `/documents` 頁、非期別頁入口）+ 上傳後 `enqueueClassification` |
...
2. `npm run dev` 手動：
   ...
   - **分類失敗 → unclassified**：弄壞 `GEMINI_API_KEY` → 文件照走；重試耗盡後 `review_status='unclassified'`、
     `error` 寫入；檢查點「未能分類：M」+1（非「待確認」）。修好金鑰按「重跑」→ 補跑後改回 accepted / pending。
   - **switch 用 target**：對一筆 pending 點「改為銷項」兩次 → 狀態穩定為 `out`（非 toggle 切回）。
   - **convert 重排 OCR**：OCR 已跑過的發票 convert 成折讓 → 顯式重新 enqueue 擷取、`extracted_data` 重算。
