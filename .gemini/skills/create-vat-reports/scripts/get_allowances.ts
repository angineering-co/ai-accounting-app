...
async function getAllowances() {
  ...
  if (!clientId || !yyymm) {
    console.error("Usage: tsx get_allowances.ts <clientId> <yyymm> [outputDir]");
    process.exit(1);
  }
  ...
  if (allowancesError) {
    console.error(`Error fetching allowances: ${allowancesError.message}`);
    process.exit(1);
  }
  ...
  const outputPath = path.join(outputDir, "allowances.json");
  fs.writeFileSync(outputPath, JSON.stringify(allowances || [], null, 2));
  console.log(`✅ Saved ${allowances?.length || 0} allowances to ${outputPath}`);
}

getAllowances().catch(console.error);
