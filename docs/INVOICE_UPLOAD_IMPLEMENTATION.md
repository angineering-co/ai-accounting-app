...
export const invoiceSchema = z.object({
  ...
  status: z.enum(['uploaded', 'processing', 'extracted', 'confirmed', 'failed']),
...
export const updateInvoiceSchema = z.object({
  ...
  status: z.enum(['uploaded', 'processing', 'extracted', 'confirmed', 'failed']).optional(),
  extracted_data: extractedInvoiceDataSchema.nullable().optional(),
});
...
export async function updateInvoice(invoiceId: string, data: UpdateInvoiceInput) {
  ...
  const validated = updateInvoiceSchema.partial().parse(data);
...
- Support file path structure: `{firmId}/{invoiceId}/{filename}`
...
### Step 9: Create Main Invoice Page
...
const uploadProps = useSupabaseUpload({
  bucketName: 'invoices',
  path: `${firmId}`, // Files will be stored in invoices/{firmId}/{filename}
