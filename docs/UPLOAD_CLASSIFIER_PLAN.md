...
| `promoteFromOther` | 低 | 交易內：建子表 + 翻 `doc_type`（`/documents` other 列表用，demote 的反向） |
...
**可用對象**：客戶（portal 佇列）為主，事務所員工（審核端）亦可用同一組動作作為後路。
兩者皆走同一 service 函式，RLS 以 firm + client 範圍隔離（Phase 6b 之 RLS）。
...
- **安靜建議（quiet suggestion）**：反向檢查的不一致**不**進 §6.2 急迫佇列、不計入徽章。
  ...
  由 `/documents` 列表（§6.5）在該 row 安靜呈現 promote 建議 chip；客戶與事務所瀏覽其他文件時
  自然看到。事務所端 `/documents`（§6.3 雙側欄）即為此類漏接的瀏覽式後路。
...
兩個入口都同時加到 `components/portal-sidebar.tsx`（client）與
`components/firm-sidebar.tsx`（事務所）：

1. **「待您確認 / 待確認分類」**（急迫佇列 §6.2 入口）：徽章顯示該 client
   `review_status='pending'` 文件數（走 §4.1 partial index）。
2. **「其他文件」**（`/documents` 列表 §6.5 入口）：徽章顯示該 client `doc_type='other'` 文件數。
   **portal 版為精簡版**：只露 promote / 刪除兩個動作，不做進階篩選 / 跨子表查找。事務所版
   同頁同動作，作為 direct-other 漏接（含安靜建議）的瀏覽式後路。
...
### 6.5 `/documents`「其他文件」列表頁（client + firm 共用，portal 精簡）
...
依角色 render：**client（portal）與 firm（事務所）皆可進**，v1 兩者皆精簡版（同一組動作）。

- **欄位**：縮圖、檔名、`doc_date`、`classifier_hint` 摘要（verdict + confidence）、操作
- **操作**：`promoteFromOther`（重分類為發票 / 折讓，需指定 in/out，會觸發 OCR）、刪除（soft delete + 清 storage）
- **安靜建議 chip**：若該 row 的 `classifier_hint.disagreed=true` 且 verdict 為 invoice/allowance
  （反向檢查，§5.6），於該列安靜呈現「看起來像發票 / 折讓，要轉嗎？」chip，點擊即走
  `promoteFromOther`。這是 direct-other 誤標的**主要修正點**，不在急迫佇列裡騷擾客戶。
...
| `lib/services/document.ts` | 加 `resolveClassificationKeep` / `switchInOrOut` / `convertDocType` / `demoteToOther` / `promoteFromOther`（Drizzle 交易）+ `getPeriodPendingClassifications` read helper |
...
| `components/portal-sidebar.tsx` / `components/firm-sidebar.tsx` | 加「待確認分類」+「其他文件」兩入口 + 徽章（皆雙側欄；其他文件 portal 精簡版）|
| `components/invoice-review-dialog.tsx` / `components/allowance-review-dialog.tsx` | 顯示 disagreed 旗標 + 審核端 demote/convert 後路 |
| `app/firm/[firmId]/client/[clientId]/documents/page.tsx` | NEW —「其他文件」列表（promote / 刪除）|
| `components/period-classification-review.tsx` | NEW — 事務所端 per-period 待確認檢查點 + 期別頁掛載 |
...
3. `npm run dev` — 手動瀏覽器驗證：
   ...
   - **direct-other 反向建議（誤標）**：選「其他文件」上傳一張統一發票 → 反向檢查覺得像 VAT →
     **不**進急迫佇列 / 不計徽章；`/documents` 該列出現安靜「要轉嗎？」chip → 點 promote → 建子表、
     `doc_type='invoice'`、OCR 觸發
   - **`/documents` promote**：在 `/documents` 對一筆 other 點「重分類為發票(進項)」→ 建子表、
     `doc_type='invoice'`、OCR 觸發、可後續 review/confirm
