 ...
 *   1. the previous period's filed `filing.summary.credit_carryover` (Field 95), else
 *   2. the posted 1145 留抵稅額 ledger balance as-of this period's start (inclusive) —
 *      catches an opening 留抵 booked at the first filing period's 期初, else
 *   3. 0.
 * The ledger fallback excludes drafts, so it assumes every prior 1145-affecting entry
 * (opening balance, prior closes) is posted. See docs/VAT_CLOSE_ENTRY_EXAMPLES.md for the
 * carryover roll-forward model.
 */
